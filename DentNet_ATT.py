...
def Batch_Normalization(x,is_train,scope='batch_norm'):
    with arg_scope([batch_norm],scope=scope,updates_collections=None,decay=0.9,center=True,scale=True,zero_debias_moving_mean=True):
        if is_train:
            return batch_norm(inputs=x,is_training=is_train,reuse=None)
        else:
            return batch_norm(inputs=x,is_training=is_train,reuse=True)
...
def convLayer(x,kHeight,kWidth,strideX,strideY,featureNum,name,pname="n",padding='SAME'):
    with tf.variable_scope(name):
        channel = int(x.get_shape()[-1])  # 获取输入的通道数
...
#prelu激活函数 注意测试时需要将is_train改为False
def prelu(_x, scope=None, name ='Prelu',is_train=True):
...
def attentional_transition(input_layer,name):
    ...
    bottom_fc = tf.nn.avg_pool(input_layer, [1, 16, 16, 1], [1, 16, 16, 1], 'VALID') #16根据特征图的大小而定
...
class DentNet_ATT:
    ...
    def build(self):
        self.conv1_1 = convLayer(self.X, 3, 3, 1, 1, 64, "conv1_1", "prelu1_1")  # 卷积核高，宽，步长，步长，输出通道
        ...
        self.trainsition1_1 = transition(self.conv1x1,self.if_a, is_train=self.is_train, keep_prob=self.keep_prob, name='transition1_1')
        ...
        self.trainsition1_2 = transition(self.conv4_3,self.if_a, is_train=self.is_train, keep_prob=self.keep_prob, name='transition1_2')
        ...
        self.trainsition1_3 =transition(self.conv5_3,self.if_a, is_train=self.is_train, keep_prob=self.keep_prob, name='transition1_3')
        ...
        self.dropout1 = dropout(self.fc1, self.keep_prob, "drop1")
        
        self.feature = fcLayer(self.dropout1, 1024, 512, "fc2", "fc2_prelu")
        self.dropout2 = dropout(self.feature, self.keep_prob, "drop2")

        self.fc3 = fcLayer(self.dropout2,512,self.CLASSNUM,"fc3")
